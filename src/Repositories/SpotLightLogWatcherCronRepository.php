...
class SpotLightLogWatcherCronRepository
{
        ...
        if(isset($entries) && sizeof($entries) > 0 )
        {
            for($i = 0 ; $i < sizeof($entries); $i++)
            {
                ...
                if(sizeof($results) > 0)
                {
                    ...
                    $emailReturn = $emailRepo->sendEmail($entries[$i]->sendTo,$entries[$i]->bcc,$entries[$i]->emailTitle,$body);
    ...
    public function buildEmailBody($logResults,$watcherName,$timeFrom)
    {
        ...
        for($i = 0;$i < sizeof($logResults) ; $i++)
        {
            ...
            {
                $addInfo = substr($logResults[$i]["entries"][0]["additionalInfo"],0,1500) . ' message has been trimed because it was over 1500 characters long';
            }
        ...
        $body = "Alle Meldungen für ihren Log Watcher <b>" . $watcherName . " </b> <br> für den Zeiraum ".$timeFrom.' - '.date('Y-m-d H:i:s' , time()) ." :<br> Insgesamte Meldungen für diesen Zeitraum:<b>".$totalCount.'</b></b><br>'.$body;
